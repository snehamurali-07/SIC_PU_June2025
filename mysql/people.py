...
class Db_operations:
    ...
    def connect_db(self):
        try:
            connection = pymysql.Connect(host='localhost', user='root', password='*********', database='people_db')
            print('DB connected')
            return connection
        except:
    ...
    def create_db(self):
        ...
        query = 'create database IF NOT EXISTS people_db;'
...
person = Person(name="John Doe", gender="M", dob="1990-01-01", location="New York")
oprs = Db_operations()
oprs.create_db()
oprs.create_table()
new_id = oprs.insert_row(person)
print("Inserted person with ID:", new_id)